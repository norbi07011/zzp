...
export const AppointmentsManager = () => {
  ...
  const filteredAppointments = useMemo(() => {
    return allAppointments.filter((apt) => {
      ...
      const appointmentDate = apt.test_date?.split("T")[0];
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-300">
              Kalendarz i harmonogram spotkań pracowników z klientami
            </p>
          </div>
          <Link
            to="/admin"
        ...
        {!loading && !error && (
          <>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              {/* QUICK APPOINTMENT - MAIN BUTTON */}
              <button
                onClick={() => setShowQuickModal(true)}
                className="px-8 py-4 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white rounded-xl font-bold text-lg transition-all flex items-center gap-3 shadow-lg hover:shadow-xl"
              >
                ➕ Nowe Spotkanie
              </button>
              ...
              <button
                onClick={() => {
                  const sample = allAppointments.find(
                    ...
                    filteredAppointments.map((apt) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          {/* Extract date and time from test_date timestamp */}
                          <div className="font-semibold text-white">
                            {apt.test_date
                              ? new Date(apt.test_date).toLocaleDateString(
                                  "pl-PL"
                                )
                              : "Brak daty"}
                          </div>
                          <div className="text-sm text-gray-400">
                            {apt.test_date
                              ? new Date(apt.test_date).toLocaleTimeString(
                                  "pl-PL",
                                  { hour: "2-digit", minute: "2-digit" }
                                )
                              : "Brak czasu"}{" "}
                            ({apt.duration_minutes || apt.duration || 0} min)
                          </div>
                          {getPriorityBadge(apt.priority)}
                        </td>
                        <td className="px-6 py-4">
                          <div className="font-semibold text-white">
                            {apt.client?.full_name || "Brak danych"}
                          </div>
                          {apt.client?.phone && (
                            <div className="text-sm text-gray-400">
                              📱 {apt.client.phone}
                        ...
                        <td className="px-6 py-4">
                          <div className="text-gray-300">
                            {apt.worker?.profile?.full_name || "Nieprzypisany"}
                          </div>
                          <div className="text-xs text-gray-500">
                            ID: {apt.worker_id?.substring(0, 8) || "N/A"}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          {apt.service_type && (
            ...
            <div className="mt-4 text-center text-gray-400">
              Wyświetlono {filteredAppointments.length} z{" "}
              {allAppointments.length} spotkań
            </div>

            {/* Enterprise Modals */}
            {/* QUICK APPOINTMENT MODAL - SIMPLE */}
            {showQuickModal && (
