...
import { RefreshCw, Mail, Inbox, AlertCircle, MessageSquare, Search, Filter } from 'lucide-react';
import type { Message, MessageCategory, MessagePriority } from '../../src/services/messages';
  ...
  const filteredMessages = useMemo(() => {
    return allMessages.filter(msg => {
      ...
      const matchesSearch = 
        ...
        msg.from_user?.profile?.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            <table className="w-full">
              <thead className="bg-white/5 border-b border-white/10">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Od</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Temat</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Kategoria</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Priorytet</th>
              ...
              <tbody>
                {filteredMessages.map((msg) => {
                  const conversationMessages = allMessages.filter(m => m.conversation_id === msg.conversation_id);
                  
                  return (
                    <tr key={msg.id} className="border-b border-white/10 hover:bg-white/5 transition-all">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-white font-medium">{msg.from_user?.profile?.full_name || 'Brak danych'}</div>
                        <div className="text-gray-400 text-sm">
                          {msg.from_user?.profile?.role === 'client' ? '👤 Klient' : 
                           msg.from_user?.profile?.role === 'worker' ? '👷 Pracownik' : '👨‍💼 Admin'}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          ...
                          msg.category === 'feedback' ? 'bg-purple-500/20 text-purple-400' :
                          'bg-gray-500/20 text-gray-400'
                        }`}>
                          {msg.category === 'inquiry' ? 'Zapytanie' :
                           msg.category === 'complaint' ? 'Reklamacja' :
                           msg.category === 'support' ? 'Wsparcie' :
                           msg.category === 'feedback' ? 'Opinia' : 'Inne'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                            ...
                            {msg.priority === 'urgent' ? 'Pilny' :
                             msg.priority === 'high' ? 'Wysoki' :
                             msg.priority === 'normal' ? 'Normalny' : 'Niski'}
                          </span>
                      ...
                      <td className="px-6 py-4 text-gray-300 text-sm">{new Date(msg.sent_at).toLocaleDateString('pl-PL')}</td>
                      <td className="px-6 py-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 text-gray-400">
                      ...
                      <span className={`px-2 py-1 rounded text-xs ${
                        ...
                        {selectedMessage.priority === 'urgent' ? 'Pilny' :
                         selectedMessage.priority === 'high' ? 'Wysoki' : 'Normalny'}
                      </span>
                    </div>
                  </div>
                  <button
