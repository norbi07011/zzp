...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Logowanie nie powiodło się';
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex justify-center mb-6">
            <Logo size="lg" showText={true} className="filter drop-shadow-2xl" />
          </div>
          ...
          <h1 className="text-4xl font-bold text-white mb-2 font-heading">Welkom terug</h1>
          <p className="text-neutral-300">Log in op uw ZZP Werkplaats account</p>
        </div>
        ...
        <div className="bg-gradient-glass backdrop-blur-md border border-accent-cyber/20 rounded-2xl shadow-3d p-8">
          ...
          {error && (
            <div className="mb-6 bg-red-50 border-l-4 border-red-500 rounded-r-lg p-4 flex items-start gap-3 animate-shake">
              ...
              <button onClick={() => setError(null)} className="text-red-500 hover:text-red-700" aria-label="Sluit foutmelding">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <svg className="h-5 w-5 text-accent-cyber" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* Password Field */}
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-white mb-2">Wachtwoord</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <svg className="h-5 w-5 text-accent-cyber" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center cursor-pointer">
                <input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} disabled={isLoading} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500 disabled:opacity-50" />
                <span className="ml-2 text-sm text-gray-700">Onthoud mij</span>
              </label>
              <Link to="/wachtwoord-vergeten" className="text-sm font-medium text-blue-600 hover:text-blue-700">Wachtwoord vergeten?</Link>
            </div>

            {/* Submit Button */}
            <button type="submit" disabled={isLoading} className="w-full flex items-center justify-center gap-2 bg-gradient-cyber hover:shadow-glow-cyber text-white font-semibold py-4 px-6 rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed shadow-glow-cyber hover:scale-105 transform disabled:transform-none">
              {isLoading ? (
                <>
        ...
        <div className="mt-6 space-y-3">
          <div className="bg-gradient-glass backdrop-blur-md border border-accent-cyber/20 rounded-xl shadow-3d p-4">
            <p className="text-sm text-neutral-300 mb-2">
              <span className="font-semibold text-accent-cyber">Opdrachtgever?</span> Registreer uw bedrijf
            </p>
            <Link to="/register/employer" className="block w-full text-center bg-accent-cyber/10 hover:bg-accent-cyber/20 text-accent-cyber font-medium py-2.5 px-4 rounded-xl transition border border-accent-cyber/30 hover:border-accent-cyber">
          ...
          <div className="bg-gradient-glass backdrop-blur-md border border-accent-techGreen/20 rounded-xl shadow-3d p-4">
            <p className="text-sm text-neutral-300 mb-2">
              <span className="font-semibold text-accent-techGreen">ZZP'er?</span> Solliciteer via ons formulier
            </p>
            <Link to="/register/worker" className="block w-full text-center bg-accent-techGreen/10 hover:bg-accent-techGreen/20 text-accent-techGreen font-medium py-2.5 px-4 rounded-xl transition border border-accent-techGreen/30 hover:border-accent-techGreen">
