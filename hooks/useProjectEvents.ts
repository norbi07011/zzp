...
export function useProjectEvents(projectId?: string) {
  ...
  // Update event
  const updateEvent = async (eventId: string, updates: Partial<ProjectEvent>) => {
    try {
      ...
      setEvents(prev => prev.map(e => e.id === eventId ? data : e).sort((a, b) => 
  ...
  const updateAttendeeResponse = async (attendeeId: string, status: 'accepted' | 'declined' | 'tentative') => {
    try {
      const { data, error: updateError } = await supabase
        .from('event_participants')
        .update({
