...
export function useAddTeamMember() {
  ...
  const sendInviteToProject = async (
    ...
    try {
      ...
      if (profileError || !profileData?.email) {
        throw new Error('Could not find user email. User must have a profile with email.');
      }
      ...
      const { data, error: insertError } = await supabase
        ...
        .insert({
          ...
          invite_message: displayName ? `Zaproszenie do projektu od ${user.email}` : undefined,
          metadata: {
  ...
  const isMemberOfProject = async (projectId: string, userId: string): Promise<boolean> => {
    try {
  ...
  const hasPendingInvite = async (projectId: string, userEmail: string): Promise<boolean> => {
    try {
      const { data } = await supabase
        .from('project_invites')
        .select('id')
