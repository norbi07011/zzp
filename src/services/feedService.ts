import { supabase, supabaseService, getCurrentUser, canUserCreatePosts } from '../lib/supabase-fixed';
...
export type PostType = 'job_offer' | 'ad' | 'announcement' | 'story' | 'service';
...
export async function createPost(postData: CreatePostData): Promise<Post> {
  ...
  console.log('[CREATE-POST] User:', user.id, 'Author type:', postData.author_type);
  ...
  console.log('[CREATE-POST] Inserting post with media:', postData.media_urls?.length || 0, 'files');
 ...
 * Update post
 */
export async function updatePost(postId: string, updates: Partial<CreatePostData>): Promise<Post> {
  const { data, error } = await supabaseAny
    .from('posts')
    .update(updates)
    .eq('id', postId)
 ...
 * Like a post
 */
export async function likePost(postId: string, userId: string, userType: UserType): Promise<void> {
  const { error } = await supabaseAny
    .from('post_likes')
    .insert({
      post_id: postId,
 ...
 * Like a comment
 */
export async function likeComment(commentId: string, userId: string, userType: UserType): Promise<void> {
  const { error } = await supabaseAny
    .from('comment_likes')
    .insert({
 ...
 */
async function getAuthorData(authorId: string, authorType: AuthorType): Promise<{
  author_name: string;
  author_company?: string;
  author_avatar?: string;
}> {
  try {
    ...
    return { author_name: authorType === 'employer' ? 'Pracodawca' : 'Księgowy' };
  }
}
 ...
 */
async function getUserData(userId: string, userType: UserType): Promise<{
  user_name: string;
  user_avatar?: string;
}> {
  try {
