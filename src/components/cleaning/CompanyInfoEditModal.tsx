...
import type { CleaningCompany, CleaningSpecialization, WeeklyAvailability, DayOfWeek } from '../../../types';
...
const DUTCH_PROVINCES = [
  ...
  'Friesland', 'Groningen', 'Drenthe', 'Flevoland', 'Zeeland', 'Limburg',
];

export const CompanyInfoEditModal: React.FC<CompanyInfoEditModalProps> = ({ company, onClose, onSave }) => {
  ...
  const specializationOptions: { value: CleaningSpecialization; label: string }[] = [
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full my-8" onClick={(e) => e.stopPropagation()}>
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-6 rounded-t-2xl flex items-center justify-between sticky top-0 z-10">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-1">Wszystkie dane w jednym miejscu</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[calc(90vh-200px)] overflow-y-auto">
          ...
          {/* LOKALIZACJA */}
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            <h3 className="text-lg font-bold text-gray-800 mb-3">📍 Lokalizacja</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Miasto *</label>
                <input type="text" value={formData.location_city} onChange={(e) => handleChange('location_city', e.target.value)}
                  className={`w-full px-4 py-2 border rounded-lg ${errors.location_city ? 'border-red-500' : 'border-gray-300'}`} />
                {errors.location_city && <p className="text-red-500 text-xs mt-1">{errors.location_city}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Prowincja</label>
                <select value={formData.location_province} onChange={(e) => handleChange('location_province', e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg">
                  <option value="">Wybierz</option>
                  {DUTCH_PROVINCES.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Promień (km)</label>
                <input type="number" min="5" max="100" value={formData.service_radius_km}
          ...
          {/* SPECJALIZACJA */}
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            <h3 className="text-lg font-bold text-gray-800 mb-3">🎯 Specjalizacja *</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {specializationOptions.map(spec => (
                <label key={spec.value} className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer ${formData.specialization.includes(spec.value) ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
                  <input type="checkbox" checked={formData.specialization.includes(spec.value)}
                    onChange={() => toggleSpecialization(spec.value)} className="w-5 h-5" />
                  <span className="font-medium text-sm">{spec.label}</span>
                </label>
              ))}
            </div>
            {errors.specialization && <p className="text-red-500 text-sm mt-2">{errors.specialization}</p>}
          </div>

          {/* DODATKOWE USŁUGI */}
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            <h3 className="text-lg font-bold text-gray-800 mb-3">✨ Dodatkowe usługi</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {additionalServicesOptions.map(srv => (
                <label key={srv.value} className={`flex items-center gap-2 p-2 rounded-lg border-2 cursor-pointer text-sm ${formData.additional_services.includes(srv.value) ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
                  <input type="checkbox" checked={formData.additional_services.includes(srv.value)}
                    onChange={() => toggleAdditionalService(srv.value)} className="w-4 h-4" />
                  <span>{srv.label}</span>
                </label>
              ))}
            </div>
          ...
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {daysOfWeek.map(day => (
                  <button key={day.key} type="button" onClick={() => toggleAvailability(day.key)}
                    className={`px-3 py-2 rounded-lg font-medium text-sm ${formData.availability[day.key] ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
                    {day.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Preferowane dni/tydzień: {formData.preferred_days_per_week}</label>
              <input type="range" min="1" max="7" value={formData.preferred_days_per_week}
                onChange={(e) => handleChange('preferred_days_per_week', parseInt(e.target.value))} className="w-full" />
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Stawka min (€/h)</label>
                <input type="number" min="10" max="100" value={formData.hourly_rate_min}
                  onChange={(e) => handleChange('hourly_rate_min', parseInt(e.target.value))}
                  className={`w-full px-4 py-2 border rounded-lg ${errors.hourly_rate_min ? 'border-red-500' : 'border-gray-300'}`} />
                {errors.hourly_rate_min && <p className="text-red-500 text-xs mt-1">{errors.hourly_rate_min}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Stawka max (€/h)</label>
                <input type="number" min="10" max="100" value={formData.hourly_rate_max}
                  onChange={(e) => handleChange('hourly_rate_max', parseInt(e.target.value))}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div className="md:col-span-2">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={formData.rate_negotiable}
                    onChange={(e) => handleChange('rate_negotiable', e.target.checked)} className="w-5 h-5" />
          ...
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Pracowników</label>
                <input type="number" min="1" max="50" value={formData.team_size}
                  onChange={(e) => handleChange('team_size', parseInt(e.target.value))}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Lat doświadczenia</label>
              ...
              <div>
                ...
                <input type="text" value={formData.kvk_number} maxLength={8}
          ...
          <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
            ...
            <textarea value={formData.bio} onChange={(e) => handleChange('bio', e.target.value)}
              rows={4} maxLength={500} className="w-full px-4 py-2 border border-gray-300 rounded-lg"
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <button type="submit" disabled={saving}
              className={`flex-1 px-6 py-3 rounded-lg font-medium flex items-center justify-center gap-2 ${saving ? 'bg-gray-300' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
