...
import { useSupabaseInvoices, useSupabaseClients, useSupabaseCompany } from '../hooks';
...
export default function Invoices({ onNavigate }: InvoicesProps) {
  ...
  const { invoices, loading: invoicesLoading, updateInvoice, deleteInvoice } = useSupabaseInvoices(user?.id || '');
  const { clients, loading: clientsLoading } = useSupabaseClients(user?.id || '');
  const { company, loading: companyLoading } = useSupabaseCompany(user?.id || '');
  ...
  const handleDeleteInvoice = async (invoiceId: string, invoiceNumber: string) => {
    if (!window.confirm(`Czy na pewno chcesz usunąć fakturę ${invoiceNumber}?`)) {
      return;
    }
              ...
              <div className="overflow-x-auto">
                <div className="min-w-full">
                  ...
                  <div className="grid grid-cols-7 gap-4 p-4 bg-gradient-to-r from-slate-100 to-gray-100 rounded-t-xl border-b border-gray-200">
                    ...
                    <div className="font-bold text-gray-700">{t.invoices.invoiceDate}</div>
                    <div className="font-bold text-gray-700">{t.invoices.dueDate}</div>
                    <div className="font-bold text-gray-700 text-right">{t.invoices.amount}</div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="relative grid grid-cols-7 gap-4 items-center">
                            ...
                            <div className="font-mono text-sm text-gray-600">{formatDate(invoice.invoice_date)}</div>
                            <div className="font-mono text-sm text-gray-600">{formatDate(invoice.due_date)}</div>
                            <div className="text-right font-mono font-bold text-gray-900">{formatCurrency(invoice.total_gross)}</div>
