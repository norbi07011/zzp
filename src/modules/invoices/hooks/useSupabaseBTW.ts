...
import type { BTWDeclaration, BTWPeriod, BTWCalculationData } from '../types/btw.js';
...
interface UseBTWReturn {
  ...
  loading: boolean;
  error: string | null;
  createDeclaration: (data: Omit<BTWDeclaration, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'output_vat_21' | 'output_vat_9' | 'total_output_vat' | 'balance'>) => Promise<BTWDeclaration>;
  updateDeclaration: (id: string, data: Partial<BTWDeclaration>) => Promise<void>;
  deleteDeclaration: (id: string) => Promise<void>;
  calculateBTW: (data: { revenue_21: number; revenue_9: number; input_vat: number }) => { output_vat_21: number; output_vat_9: number; total_output_vat: number; balance: number };
...
export function useSupabaseBTW(userId: string, year?: number): UseBTWReturn {
  ...
  const fetchDeclarations = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await query.order('year', { ascending: false }).order('quarter', { ascending: false });
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch BTW declarations');
    } finally {
      setLoading(false);
    }
  }, [userId, year]);
  ...
  // CALCULATE BTW
  // =====================================================
  const calculateBTW = (data: { revenue_21: number; revenue_9: number; input_vat: number }) => {
  ...
  const createDeclaration = async (
    data: Omit<BTWDeclaration, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'output_vat_21' | 'output_vat_9' | 'total_output_vat' | 'balance'>
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to create BTW declaration';
      setError(errorMsg);
  ...
  // UPDATE DECLARATION
  // =====================================================
  const updateDeclaration = async (id: string, updates: Partial<BTWDeclaration>): Promise<void> => {
    try {
      setError(null);
      ...
      let calculated = {};
      if (updates.revenue_21 !== undefined || updates.revenue_9 !== undefined || updates.input_vat !== undefined) {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to update BTW declaration';
      setError(errorMsg);
      throw new Error(errorMsg);
    }
  };
  ...
  const deleteDeclaration = async (id: string): Promise<void> => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to delete BTW declaration';
      setError(errorMsg);
      throw new Error(errorMsg);
    }
  };
