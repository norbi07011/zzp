...
export type FileType = 'image' | 'document' | 'spreadsheet' | 'presentation' | 'archive' | 'video' | 'audio' | 'cad' | 'blueprint' | 'other';
export type FileStatus = 'uploading' | 'active' | 'processing' | 'archived' | 'deleted';
...
export const useFileManager = ({ projectId, enabled = true }: UseFileManagerProps): UseFileManagerReturn => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Nieznany błąd podczas pobierania plików';
  ...
  const uploadFiles = useCallback(async (filesToUpload: File[]): Promise<ProjectFile[]> => {
    ...
    try {
        ...
        try {
          ...
          if (existingFile) {
            setUploadProgress(prev => prev.map(p => 
              p.fileId === fileId 
                ? { ...p, status: 'error', error: `Plik już istnieje: ${existingFile.original_filename}` }
                : p
            ));
            continue;
          }
          ...
          const errorMessage = fileError instanceof Error ? fileError.message : 'Nieznany błąd uploadu';
          setUploadProgress(prev => prev.map(p => 
            p.fileId === fileId 
      ...
      const errorMessage = err instanceof Error ? err.message : 'Nieznany błąd uploadu';
      setError(errorMessage);
      setUploadProgress([]);
      return [];
    }
  ...
  const downloadFile = useCallback(async (file: ProjectFile): Promise<void> => {
    try {
      setError(null);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Nieznany błąd pobierania';
      setError(errorMessage);
  ...
  const deleteFile = useCallback(async (fileId: string): Promise<void> => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Nieznany błąd usuwania';
      setError(errorMessage);
  ...
  const updateFile = useCallback(async (fileId: string, updates: Partial<ProjectFile>): Promise<void> => {
    try {
      setError(null);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Nieznany błąd aktualizacji';
      setError(errorMessage);
  ...
  const getFileUrl = useCallback(async (file: ProjectFile, download = false): Promise<string> => {
    try {
  ...
  const generateThumbnail = useCallback(async (file: ProjectFile): Promise<string | null> => {
