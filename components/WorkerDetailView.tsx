...
export const WorkerDetailView: React.FC<WorkerDetailViewProps> = ({ workerId: propWorkerId }) => {
  ...
  const loadWorkerData = async () => {
    ...
    try {
      ...
      setPortfolio([
        { id: 1, title: 'Projekt 1', image_url: '/api/placeholder/300/200', description: 'Remont kuchni' },
        { id: 2, title: 'Projekt 2', image_url: '/api/placeholder/300/200', description: 'Łazienka' },
      ]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-primary-dark flex items-center justify-center">
        <div className="text-white text-xl">⏳ Ładowanie profilu pracownika...</div>
      </div>
    );
  }
  ...
  if (!worker) {
    return (
      <div className="min-h-screen bg-primary-dark flex items-center justify-center">
        <div className="text-red-400 text-xl">❌ Pracownik nie został znaleziony</div>
      </div>
    );
  }
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-3xl font-bold text-white">{worker.full_name}</h1>
                {worker.verified && (
              ...
              <div className="flex items-center gap-6 text-neutral-300">
                <div className="flex items-center gap-2">
                  ...
                  <span>{worker.rating.toFixed(1)} ({worker.rating_count} opinii)</span>
                </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-dark-800 rounded-xl p-6 border border-neutral-700">
              ...
              {portfolio.length > 0 ? (
                <div className="grid md:grid-cols-2 gap-6">
                  {portfolio.map((item) => (
                    <div key={item.id} className="bg-dark-700 rounded-lg overflow-hidden border border-neutral-600">
                      <img 
                        src={item.image_url} 
                        alt={item.title}
          ...
          <div className="space-y-6">
            {/* Skills */}
            <div className="bg-dark-800 rounded-xl p-6 border border-neutral-700">
              <h3 className="text-lg font-bold text-white mb-4">🛠️ Umiejętności</h3>
              {worker.skills && worker.skills.length > 0 ? (
                ...
                <div className="text-neutral-400 text-sm">Brak umiejętności</div>
              )}
            </div>

            {/* Certificates */}
            <div className="bg-dark-800 rounded-xl p-6 border border-neutral-700">
              <h3 className="text-lg font-bold text-white mb-4">🏆 Certyfikaty</h3>
              {certificates.length > 0 ? (
                <div className="space-y-3">
                  {certificates.map((cert) => (
                    <div key={cert.id} className="p-3 bg-dark-700 rounded-lg border border-neutral-600">
                      <div className="text-white font-semibold">{cert.name}</div>
                      <div className="text-neutral-400 text-sm">{cert.issuer}</div>
                      {cert.date_earned && (
                        <div className="text-neutral-500 text-xs mt-1">
                          Uzyskano: {new Date(cert.date_earned).toLocaleDateString('pl-PL')}
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="text-neutral-400 text-sm">Brak certyfikatów</div>
              )}
            </div>
      ...
      {showReviewForm && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-8">
          <div className="bg-dark-800 rounded-xl p-6 max-w-md w-full border border-neutral-700">
            <h3 className="text-xl font-bold text-white mb-4">⭐ Dodaj opinię</h3>
            <ReviewForm
              workerId={workerId!}
              employerId="temp-employer-id" // TODO: Get current employer ID
