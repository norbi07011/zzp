...
export const PageContainer = ({ 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      <div className={`${maxWidthClasses[maxWidth]} mx-auto p-6 space-y-8 ${className}`}>
        {children}
      </div>
    </div>
  );
...
export const StatCard = ({ title, value, icon, color }: StatCardProps) => {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-white/80 backdrop-blur-sm border border-white/30 p-6 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
      ...
      <div className={`absolute -bottom-2 -right-2 w-16 h-16 ${colors.glow} rounded-full blur-lg`}></div>
    </div>
  );
};
...
export const ContentCard = ({ children, className = '', noPadding = false }: ContentCardProps) => {
  return (
    <div className={`relative overflow-hidden rounded-2xl bg-white/80 backdrop-blur-sm border border-white/30 shadow-xl ${className}`}>
