...
type TabType = 'basic' | 'photos' | 'materials' | 'checklist' | 'cost' | 'template';
...
export function TaskFormModal({ isOpen, onClose, projectId, task, onSave }: TaskFormModalProps) {
  const { createTask, updateTask, applyTemplate, fetchTemplates, loading } = useProjectTasks(projectId);
  ...
  const handleTemplateSelect = async (template: TaskTemplate) => {
    ...
    if (!task?.id) {
      ...
      alert('Szablon zastosowany lokalnie! Zapisz zadanie aby zachować zmiany.');
      return;
    }
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'basic' && (
          <div className="space-y-4">
            ...
            {/* Building Address */}
            <div>
              <label htmlFor="building_address" className="block text-sm font-medium text-gray-700 mb-1">
                📍 Adres placu budowy
              </label>
              <input
                id="building_address"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="room_floor" className="block text-sm font-medium text-gray-700 mb-1">
                  Piętro / Pokój
                </label>
                <input
                  id="room_floor"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleChange('priority', e.target.value as ProjectTask['priority'])}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleChange('status', e.target.value as ProjectTask['status'])}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
            ...
            <div>
              ...
              {loadingMembers && (
                <p className="mt-1 text-sm text-gray-500">Ładowanie członków zespołu...</p>
              )}
              {!loadingMembers && members.length === 0 && (
                <p className="mt-1 text-sm text-amber-600">Brak członków zespołu. Dodaj członków w zakładce Team.</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="0.5"
                  value={formData.estimated_hours ? (formData.estimated_hours / 8).toFixed(1) : ''}
                  onChange={(e) => {
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Koszt robocizny: €{((formData.hourly_rate || 0) * (formData.estimated_hours || 0)).toFixed(2)}
              </p>
            </div>
          </div>
        )}
            ...
            {!task?.id && (
              <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-sm text-amber-800">
                  ⚠️ Szablon można w pełni zastosować dopiero po zapisaniu zadania. 
                  Na razie możesz podglądać szablony.
                </p>
              </div>
            )}
          </div>
            ...
            {!task?.id && (
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  💡 Zdjęcia możesz dodać lokalnie. Zostaną przesłane po zapisaniu zadania.
                </p>
              </div>
            )}
          </div>
        ...
        {activeTab === 'materials' && (
          <div>
            <TaskMaterialsList
              ...
              onMaterialsChange={(materials) => handleChange('materials', materials)}
            />
            {!task?.id && (
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  💡 Materiały są zapisywane lokalnie i zostaną przesłane po zapisaniu zadania.
                </p>
              </div>
            )}
          </div>
        ...
        {activeTab === 'checklist' && (
          <div>
            <TaskChecklistManager
              ...
              onChecklistChange={(checklist) => handleChange('checklist', checklist)}
            />
            {!task?.id && (
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  💡 Checklist jest zapisywany lokalnie i zostanie przesłany po zapisaniu zadania.
                </p>
              </div>
            )}
          </div>
        ...
        {activeTab === 'cost' && (
          <div>
            <TaskCostCalculator
              ...
              onEstimatedHoursChange={(hours) => handleChange('estimated_hours', hours)}
            />
            {!task?.id && (
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  💡 Koszty są obliczane lokalnie. Dane zostaną zapisane po zatwierdzeniu zadania.
                </p>
              </div>
            )}
          </div>
