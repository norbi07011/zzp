 ...
 */
export function AddToTeamButton({ userId, userEmail, userType, displayName, avatarUrl, className = '' }: AddToTeamButtonProps) {
  const { loading, error, fetchMyProjects, sendInviteToProject, isMemberOfProject, hasPendingInvite } = useAddTeamMember();
  ...
  const handleAddToProject = async (projectId: string, projectName: string) => {
    try {
      ...
      const roleMapping: Record<'worker' | 'employer' | 'accountant', 'member' | 'admin'> = {
      ...
      await sendInviteToProject(projectId, userId, projectRole, displayName, avatarUrl);
          ...
          <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-200 dark:border-slate-700 z-40 overflow-hidden">
            ...
            <div className="max-h-64 overflow-y-auto">
              {projects.map((project) => {
                ...
                return (
                  <button
                    key={project.id}
                    onClick={() => canInvite && handleAddToProject(project.id, project.name)}
                    ...
                    className={`
                      ...
                      ${!canInvite ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}
                    `}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`
                            ...
                            ${project.status === 'paused' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : ''}
                          `}>
                      ...
                      {isMember ? (
                        ...
                        <div className="flex items-center gap-1.5 ml-3 text-blue-600 dark:text-blue-400">
                          ...
                          <span className="text-xs font-medium">Zaproszono</span>
                        </div>
                      ) : (
                        <div className="ml-3">
            ...
            {projects.length === 0 && (
              <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                ...
                <p className="text-xs mt-1">Utwórz projekt aby dodawać członków zespołu.</p>
              </div>
            )}
          </div>
        </>
